...
import {
	...
	ConversationModel as Conversation, EventModel as Event,
...
import { UserStatusEnum, EventTypeEnum } from '../enum';
import {
	ReturnCodeEnum,
	ReturnMessage,
	ReturnMessages
...
import {
	UserNotFoundError,
	UserNoSocketError, PropertyUndefinedError
} from '../error';
...
export class ChatManager {
	...
	*/
	...
	public async connectUser(sourceUser: User): Promise<Response> {
		...
		if (sourceUser.isDisabled) return Response.AccountDisabled();
		if (sourceUser.isBanned) return Response.BannedFromSite();

		//TODO falta la gestión de más de una conexión simultánea con el mismo usuario
		this.usersBySocket_.set(sourceUser.socket, sourceUser);
		this.updateUser_(sourceUser, { status: UserStatusEnum.ONLINE });
		this.raise_<void>('onUserConnected', { sourceUser });
		return Response.Success();
	}

	public async disconnectUser(sourceUser: User): Promise<void> {
		this.updateUser_(sourceUser, { status: UserStatusEnum.OFFLINE });
	...
	public changeNickUserUUID(userUUID: string, newNick: string): void {
		const user = this.getUserByUUID(userUUID);

		if (!user) return;
		this.updateUser_(user, {
			name: newNick,
		});
	}
	...
	private async createChannel_(sourceUser: User, channelName: string, password?: string): Promise<Channel> {
		...
		const channel = this.addChannelDB(channelDB);
		this.raise_<void>('onChannelCreated', { channel, sourceUser });
		return channel;
	}

	private async deleteChannel_(channel: Channel) {
		await this.channelsService_.remove(channel.uuid);
	...
	private addGenericEvent(channel: Channel, type: EventTypeEnum, sourceUser: User): Event {
		const event = channel.addGenericEvent(type, sourceUser);

		this.raise_<void>('onEventCreated', { channel, event });
		return event;
	}
	...
	public async createChannelName(sourceUser: User, channelName: string, password?: string): Promise<Response> {
		...
		channel = await this.createChannel_(sourceUser, channelName, password);
		this.addGenericEvent(channel, EventTypeEnum.CREATE, sourceUser);

		return Response.Success();
	}

	public async deleteChannelUUID(sourceUser: User, channelUUID: string): Promise<Response> {
		const channel = this.getChannelByUUID(channelUUID);

		if (!channel) return Response.ChannelNotExists();
		if (!channel.isOwner(sourceUser)) Response.InsufficientPrivileges();

		this.addGenericEvent(channel, EventTypeEnum.CLOSE, sourceUser);
	...
	public async joinChannelUUID(sourceUser: User, channelUUID: string, password?: string): Promise<Response> {
		...
		*/
		...
		const event = channel.addGenericEvent(EventTypeEnum.JOIN, sourceUser);
		//this.raise_<void>('onUserJoined', { channel, sourceUser, event });
		return Response.Success();
	}
	...
	public async partChannelUUID(sourceUser: User, channelUUID: string): Promise<Response> {
		...
		const event = channel.addGenericEvent(EventTypeEnum.PART, sourceUser);
		if (channel.isEmpty)
	...
	public async observeUserUUID(sourceUser: User, targetUserUUID: string): Promise<Response> {
		...
		this.updateUser_(sourceUser, { status: UserStatusEnum.IN_GAME });
	...
	public async closeChannelUUID(sourceUser: User, channelUUID: string, message: string): Promise<Response> {
		...
		if (!channel) return Response.ChannelNotExists();
		if (!channel.hasPrivileges(sourceUser) && !sourceUser.hasPrivileges()) Response.InsufficientPrivileges();
	...
	public updateUser_(user: User, updatedValues: any) {
		if (updatedValues) {
			this.logger_.log(`${user.name} status changed to ${['Offline', 'Online', 'In game', 'Away'][updatedValues]}`);
			this.raise_<void>('userUpdated', { user, updatedValues });
		}
	}
	/*
	...
	*/
	private userFromDB_(userDB: UserDB): User {
		//TODO: El nick a veces viene vacío
		if (!userDB.nickname)
